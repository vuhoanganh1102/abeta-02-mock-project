...
export class Migrations1716364660202 implements MigrationInterface {
  name = 'Migrations1716364660202';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`attendance\` (\`id\` bigint UNSIGNED NOT NULL AUTO_INCREMENT, \`user_id\` bigint UNSIGNED NOT NULL, \`date\` date NOT NULL, \`checkIn\` timestamp NULL, \`checkOut\` timestamp NULL, \`workHours\` float NULL, \`lateTime\` float NULL, \`status\` int UNSIGNED NOT NULL COMMENT '1: active, 0: deleted, 2: pending, 3: reject' DEFAULT '1', \`deleted_at\` datetime(6) NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`notification\` (\`id\` bigint UNSIGNED NOT NULL AUTO_INCREMENT, \`title\` varchar(255) NOT NULL, \`content\` varchar(255) NOT NULL, \`admin_id\` bigint UNSIGNED NOT NULL, \`deleted_at\` datetime(6) NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`refresh_token\` varchar(500) NULL`,
    );
    await queryRunner.query(`ALTER TABLE \`user\` DROP COLUMN \`reset_token\``);
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`reset_token\` varchar(500) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`admin\` DROP COLUMN \`refresh_token\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`admin\` ADD \`refresh_token\` varchar(500) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`admin\` DROP COLUMN \`reset_token\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`admin\` ADD \`reset_token\` varchar(500) NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`admin\` ADD \`refresh_token\` varchar(255) NULL`,
    );
    await queryRunner.query(`ALTER TABLE \`user\` DROP COLUMN \`reset_token\``);
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`reset_token\` varchar(255) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` DROP COLUMN \`refresh_token\``,
    );
