...
export class Migrations1716196386246 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`firstName\` varchar(255) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`lastName\` varchar(255) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`phoneNumber\` varchar(20) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD UNIQUE INDEX \`IDX_f2578043e491921209f5dadd08\` (\`phoneNumber\`)`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`dateOfBirth\` varchar(255) NULL`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`resetToken\` varchar(255) NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`status\` tinyint UNSIGNED NOT NULL DEFAULT '1'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`reset_token\` varchar(100) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`refresh_token\` varchar(500) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`phone_number\` varchar(100) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`name\` varchar(100) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`is_verified\` tinyint UNSIGNED NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
